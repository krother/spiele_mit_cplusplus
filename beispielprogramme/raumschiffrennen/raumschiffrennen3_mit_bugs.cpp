...
class Raumschiff {
    ...
    virtual void zeichnen(SDL_Surface *schiffe, SDL_Surface *surf) {
        SDL_Rect = {x: kachelx, y: kachely, w:32, h:32};
        SDL_Rect = {x: posx, y: posy, w:32, h:32};
...
vector<Raumschiff*> schiffe_erzeugen(anzahl) {
    ...
    if ( SDL_PollEvent( &e ) != 0 ) { 
        ...
        if( e.type == SDL_KEYDOWN ) { 
            switch (e.key.keysym.sym) {
                case SDLK_UP: spieler->oben();
                case SDLK_DOWN: spieler->unten();
                case SDLK_LEFT: spieler->links();
...
void zeichnen(vector<Raumschiff*> schiffe, Raumschiff spieler, SDL_Surface *raumschiffe, SDL_Surface *surf) {
    for (unsigned int j=1; j<schiffe.size(); j++) {
        if (!schiffe[j]->explodiert) {
